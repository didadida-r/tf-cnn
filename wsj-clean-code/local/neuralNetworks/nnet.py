...
class Nnet(object):
    ...
    def __init__(self, conf, input_dim, num_labels):
        ...
        self.ACC_RESET_EPOCH = int(self.conf['acc_reset_epochs'])
        ...
        with tf.Session(graph=trainer.graph, config=config):
            ...
            while step < num_steps:
                ...
                if (step%int(self.conf['valid_frequency']) == 0
                    ...
                    current_loss, current_acc = trainer.evaluate(val_data, val_labels)
                    print('validation loss at step %d: %f, the acc is: %f' %(step, current_loss, current_acc))
                ...
                if int(self.conf['add_layer_period']) > 0:
                    if (step%int(self.conf['add_layer_period']) == 0
                        ...
                        #do a validation step
                        validation_loss, validation_acc = trainer.evaluate(val_data, val_labels)
                        print("validation loss at step %d: %f, the acc is: %f" % (
                            step, validation_loss, validation_acc))
                        validation_step = step
                        trainer.save_trainer(self.conf['savedir']
                                             + '/training/validated')
