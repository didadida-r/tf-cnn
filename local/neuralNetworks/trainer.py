...
class Trainer(object):
    ...
    def __init__(self, classifier, input_dim, max_input_length,
        ...
        with self.graph.as_default():
            ...
            batch_acc = tf.get_variable('batch_acc', [], dtype=tf.float32,
                initializer=tf.constant_initializer(0), trainable=False)
            ...
            with tf.name_scope('train'):
                ...
                #clip the gradients
                meangrads = [tf.clip_by_value(grad, -1., 1., name=grad.op.name)
            ...
            with tf.name_scope('valid'):
                self.init_acc = batch_acc.initializer

                #compute the validation loss
                valid_loss, true_count = self.compute_loss(
                ...
                self.update_valid_loss = tf.group(
                    *([batch_loss.assign_add(valid_loss), batch_acc.assign_add(true_count) ,update_num_frames]))

            #operation to compute the average loss in the batch
            self.average_loss = batch_loss/tf.cast(num_frames, tf.float32)
            self.average_acc = batch_acc/tf.cast(num_frames, tf.float32)
    ...
    def update(self, inputs, targets):
        ...
        input_seq_length = [i.shape[0] for i in inputs]     # 得到一个list，代表每句话的帧长
        ...
        for k in range(int(len(added_inputs)/self.numutterances_per_minibatch)):
...
class CrossEnthropyTrainer(Trainer):
    ...
    def compute_loss(self, targets, logits, logit_seq_length,
        ...
        with tf.name_scope('cross_enthropy_loss'):
            ...
            # Evaluate model
            # argmax取的是最大值的下标
            correct_pred = tf.equal(tf.argmax(nonseq_logits, 1), tf.argmax(nonseq_targets, 1))
